...
import { ROLES_KEY, REQUIRE_ALL_ROLES_KEY } from '../decorators/roles.decorator';
...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const requiredAllRoles = this.reflector.get<string[]>(REQUIRE_ALL_ROLES_KEY, context.getHandler()) || [];
    ...
    const hasAllRequiredRoles = requiredAllRoles.length === 0 || 
